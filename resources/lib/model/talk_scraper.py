...
def get(html, logger, video_quality='320kbps'):
    ...
    init_scripts = [script for script in xbmc_common.parseDOM(
        html, 'script') if "'talkPage.init'" in script]
    if init_scripts:

        logger('%s = %s' % (str(init_scripts)), level='debug')
        ...
        logger('%s = %s' % ('init_scripts altered', str(init_scripts)), level='debug')
        ...
        logger('%s = %s' % ('init_scripts sliced', str(init_scripts)), level='debug')
        ...
        logger('%s = %s' % ('title', str(title)), level='debug')

        speaker = talk_json['player_talks'][0]['speaker']

        logger('%s = %s' % ('speaker', str(speaker)), level='debug')

        resources = talk_json['player_talks'][0]['resources']

        logger('%s = %s' % ('resources', str(json.dumps(resources))), level='debug')
        ...
        logger('%s = %s' % ('url', str(url)), level='debug')

        plot = talk_json['description']

        logger('%s = %s' % ('plot', str(plot)), level='debug')
